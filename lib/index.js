...
/**
 * Get the extension from a filename, if any.
 * @method getExt
 * @private
...
var loadConfigurations = function() {
		...
		switch(getExt(file)) {
			...
			case ".js":
				...
				if(loaded[file].resolve /*instanceof Function*/) {
					resolve.push(loaded[file]);
				}
				break;
			case ".example":
				// Ignoring example files
				break;
			default:
	...
	for(i=0; i<process.argv.length; i++) {
		if(process.argv[i] === "--config:global") {
			argument.global = process.argv[++i];
		}
		if(process.argv[i] === "--config" || process.argv[i] === "--config:local") {
			argument.local = process.argv[++i];
		}
		if(process.argv[i] === "--config:ignore" || process.argv[i] === "--ignore") {
			argument.ignore = process.argv[++i];
		}
	}
	...
	appPackage.aconfiguration.locals = argument.local || appPackage.aconfiguration.application || appPackage.aconfiguration.app || appPackage.aconfiguration.locals || appPackage.aconfiguration.directory || "app/configuration/";
	appPackage.aconfiguration.globals = argument.global || appPackage.aconfiguration.system || appPackage.aconfiguration.sys || appPackage.aconfiguration.globals;
	appPackage.aconfiguration.final = appPackage.aconfiguration.final || appPackage.aconfiguration.finish;
	appPackage.aconfiguration.ignore = argument.ignore || appPackage.aconfiguration.ignore || "_";

	if(!fs.lstatSync(appPackage.aconfiguration.locals).isDirectory()) {
		throw new Error("The indicated local directory is not a directory: " + appPackage.aconfiguration.locals);
	}
	if(appPackage.aconfiguration.globals && !fs.lstatSync(appPackage.aconfiguration.globals).isDirectory()) {
		throw new Error("The indicated global directory is not a directory: " + appPackage.aconfiguration.globals);
	}
	...
	try {
		...
		resolve.forEach(function(file) {
			resolveConfiguration(file.resolve(module.exports));
		});
